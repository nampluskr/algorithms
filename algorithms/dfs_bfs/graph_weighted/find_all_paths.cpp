...
void dfs_preorder(int curr, int target) {
    if (visited[curr]) return;
    // visit the current node (preorder)
    visited[curr] = true; path.push_back(curr);
    if (curr == target) { print(path); return; }

    for (auto& next: adjList[curr])
        if (!visited[next.to]) {
            parent[next.to] = curr;
            dfs_preorder(next.to, target);
            // backtrack
...
void dfs_postorder(int curr, int target) {
    if (curr == target) { print(path); return; }
    ...
    for (auto& next: adjList[curr])
        if (!visited[next.to]) {
            ...
            parent[next.to] = curr;
            dfs_postorder(next.to, target);
            // backtrack
            visited[next.to] = false; path.pop_back();
        }
}

void bfs_preorder(int start, int target) {
    std::queue<Edge> Q;
    ...
    while (!Q.empty()) {
        ...
        if (curr.to == target) { print(curr.path); continue; }
...
// visited[start] = true; path.push_back(start);
void bfs_postorder(int start, int target) {
    std::queue<Edge> Q;
    Q.push({ start, 0, visited, path });
